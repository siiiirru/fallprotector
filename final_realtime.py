        ...
        if image is not None:
            ...
            if FALL_COUNTER>=1:
                ...
                sendMsg("The Fall has been occurred!",
                        time.strftime('%Y-%m-%d %H:%M:%S'),resized_image,"qw803qw803@gmail.com")
                FALL_COUNTER=0
        
        
            #show_image=cv2.resize(resized_image,VIEW_SIZE)
            #cv2.imshow("Processed Image",show_image)
            #cv2.waitKey(1)
